...
blitTextures buffSize (tex:texs) = foldl' combine (blitTextures buffSize [tex]) texs
  where
    ...
    combine acc (SomeTexture blitTex, origin, size) = fromFrameBufferColor RGBA8 buffSize $ blitTexture acc blitTex origin size
...
blitTexture :: (ColorFormat f1, ColorFormat f2) => 
  ...
  -> FrameBuffer RGBAFormat () ()
blitTexture base tex origin size = paintSolidAlpha (textureQuad' base tex origin size) emptyFrameBufferAlpha
